...
Deliver held-out evaluation for bounded AI planning in isolated Web Cyber Range.
...
Topic-specific artifact for held-out evaluation.
...
Reviewable held-out evaluation artifact with explicit status and evidence boundary.
...
Produce no tuning heldout; preserve source hierarchy and status vocabulary.
...
Focused check covers no tuning heldout; main validator and unit tests deferred to validator repair.

## Acceptance Criteria

No tuning heldout exists as topic-specific reviewable artifact; no VALIDATED claim without required review and evidence.
