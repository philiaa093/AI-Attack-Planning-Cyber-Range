...
Deliver action catalog for bounded AI planning in isolated Web Cyber Range.
...
Topic-specific artifact for action catalog.
...
Reviewable action catalog artifact with explicit status and evidence boundary.
...
Produce finite IDs/no arbitrary shell; preserve source hierarchy and status vocabulary.
...
Focused check covers finite IDs/no arbitrary shell; validated by scaffold, contract, safety, planner, and evaluation tests recorded in `EV-FOUNDATION-001-001` through `EV-FOUNDATION-001-004`.

## Acceptance Criteria

Finite ids/no arbitrary shell exists as topic-specific reviewable artifact; validated by required checks, immutable hashed receipts, and final read-only approval.
