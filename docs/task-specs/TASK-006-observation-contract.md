...
Deliver observation contract for bounded AI planning in isolated Web Cyber Range.
...
Topic-specific artifact for observation contract.
...
Reviewable observation contract artifact with explicit status and evidence boundary.
...
Produce observation schema; preserve source hierarchy and status vocabulary.
...
Focused check covers observation schema; validated by scaffold, contract, safety, planner, and evaluation tests recorded in `EV-FOUNDATION-001-001` through `EV-FOUNDATION-001-004`.

## Acceptance Criteria

Observation schema exists as topic-specific reviewable artifact; validated by required checks, immutable hashed receipts, and final read-only approval.
