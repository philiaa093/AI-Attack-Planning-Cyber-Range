...
Deliver cyber range network for bounded AI planning in isolated Web Cyber Range.
...
Topic-specific artifact for cyber range network.
...
Reviewable cyber range network artifact with explicit status and evidence boundary.
...
Produce isolation test; preserve source hierarchy and status vocabulary.
...
Focused check covers isolation test; main validator and unit tests deferred to validator repair.

## Acceptance Criteria

Isolation test exists as topic-specific reviewable artifact; no VALIDATED claim without required review and evidence.
