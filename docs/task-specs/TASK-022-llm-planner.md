...
Deliver llm planner for bounded AI planning in isolated Web Cyber Range.
...
Topic-specific artifact for llm planner.
...
Reviewable llm planner artifact with explicit status and evidence boundary.
...
Produce structured action IDs; preserve source hierarchy and status vocabulary.
...
Focused check covers structured action IDs; main validator and unit tests deferred to validator repair.

## Acceptance Criteria

Structured action ids exists as topic-specific reviewable artifact; no VALIDATED claim without required review and evidence.
