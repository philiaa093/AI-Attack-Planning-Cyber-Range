...
Deliver clean control target for bounded AI planning in isolated Web Cyber Range.
...
Topic-specific artifact for clean control target.
...
Reviewable clean control target artifact with explicit status and evidence boundary.
...
Produce known negative; preserve source hierarchy and status vocabulary.
...
Focused check covers known negative; main validator and unit tests deferred to validator repair.

## Acceptance Criteria

Known negative exists as topic-specific reviewable artifact; no VALIDATED claim without required review and evidence.
