...
Deliver evidence contract for bounded AI planning in isolated Web Cyber Range.
...
Topic-specific artifact for evidence contract.
...
Reviewable evidence contract artifact with explicit status and evidence boundary.
...
Produce append-only evidence; preserve source hierarchy and status vocabulary.
...
Focused check covers append-only evidence; validated by scaffold, contract, safety, planner, and evaluation tests recorded in `EV-FOUNDATION-001-001` through `EV-FOUNDATION-001-004`.

## Acceptance Criteria

Append-only evidence exists as topic-specific reviewable artifact; validated by required checks, immutable hashed receipts, and final read-only approval.
