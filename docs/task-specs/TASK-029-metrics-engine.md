...
Deliver metrics engine for bounded AI planning in isolated Web Cyber Range.
...
Topic-specific artifact for metrics engine.
...
Reviewable metrics engine artifact with explicit status and evidence boundary.
...
Produce metric tests; preserve source hierarchy and status vocabulary.
...
Focused check covers metric tests; main validator and unit tests deferred to validator repair.

## Acceptance Criteria

Metric tests exists as topic-specific reviewable artifact; no VALIDATED claim without required review and evidence.
